# --- Описание действий ---
# 1. Сначала проверяется и запрашивается текущее состояние системы/папок через prepare, чтобы обеспечить возможность отката/очистки.
# 2. Выполняется трансляция Vala в Си и компиляция через gcc, после чего собирается исполняемый файл.
...
# Флаги для GCC
CFLAGS = $(shell pkg-config --cflags gtk+-3.0) -O2 -fno-ident
...
# Создание только необходимых для сборки папок с предварительным контролем состояния
prepare:
	@echo "Проверка состояния папок сборки..."
	@mkdir -p $(OUT_DIR)
	@mkdir -p $(OBJ_DIR)/$(SRC_DIR)

# Безопасная точечная очистка
clean:
	@echo "Выполнение очистки временных файлов..."
